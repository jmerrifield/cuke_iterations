...
describe CukeParser do
  subject { CukeParser }
  describe 'parsing a feature files content' do
    before do
      content = File.read(File.join(File.dirname(__FILE__), '..', 'example', 'features', 'cuking_rocks.feature'))
      @scenarios = subject.parse_feature_file_content(content)
    end

    it 'should find all the scenarios and examples' do
      @scenarios.map { |x| x[:line] }.should =~ [10, 21, 22]
    end
  end

  describe 'parsing a features folder' do
    before do
      features_dir = File.join(File.dirname(__FILE__), '..', 'example', 'features')

      @scenarios = subject.parse_features(features_dir)
    end
